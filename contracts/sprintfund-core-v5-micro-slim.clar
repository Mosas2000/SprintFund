;; SprintFund Core v5-micro-slim
;; Configurable micro-stake DAO - Optimized for deployment
...
(define-public (create-proposal (amount uint) (title (string-utf8 100)) (description (string-utf8 500)))
  (let
    ...
    (print { event: "proposal-created", proposal-id: new-proposal-id })
    (ok new-proposal-id)
  )
)
...
(define-public (vote (proposal-id uint) (support bool) (vote-weight uint))
  (let
    ...
    (map-set stakes
      { staker: tx-sender }
      { amount: (get amount voter-stake), locked-until: (+ stacks-block-height STAKE-LOCKUP-BLOCKS) }
    )
    (map-set votes
    ...
    (print { event: "vote", proposal-id: proposal-id, voter: tx-sender })
    (ok true)
  )
)
...
(define-public (reclaim-vote-cost (proposal-id uint))
  (let
    ...
    (print { event: "vote-cost-reclaimed", proposal-id: proposal-id })
    (ok true)
  )
)
