;; SprintFund Core Contract (v5-minimal)
;; Based on proven v4-minimal + configurable micro-stake
...
(define-data-var min-stake-amount uint u100) ;; 100 microSTX (0.0001 STX), configurable
...
(define-public (withdraw-stake (amount uint))
  (let
    (
      ...
      (withdrawal-recipient tx-sender)
    )
    ...
    (var-set total-staked (- (var-get total-staked) amount))
    (try! (as-contract (stx-transfer? withdrawal-amount tx-sender withdrawal-recipient)))
...
(define-public (execute-proposal (proposal-id uint))
  (let
    (
      ...
      (proposal-proposer (get proposer proposal))
    )
    (asserts! (is-eq tx-sender proposal-proposer) ERR-NOT-AUTHORIZED)
    (asserts! (not (get executed proposal)) ERR-ALREADY-EXECUTED)
    ...
    (try! (as-contract (stx-transfer? proposal-amount tx-sender proposal-proposer)))
