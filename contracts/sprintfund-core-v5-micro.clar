;; SprintFund Core Contract (v5-micro)
;; Production-ready DAO with configurable micro-stake support
;; All security features from v4-minimal + flexible minimum stake
...
(define-data-var min-stake-amount uint u100) ;; Default: 100 microSTX (0.0001 STX), configurable
...
;; Stake STX - configurable minimum
(define-public (stake (amount uint))
  (let
    ...
    (print { event: "stake", staker: tx-sender, amount: amount, new-total: new-stake-amount })
    (ok new-stake-amount)
  )
)
...
(define-public (withdraw-stake (amount uint))
  (let
    ...
    (print { event: "unstake", staker: tx-sender, amount: amount, remaining: (- stake-amount amount) })
...
(define-public (create-proposal (amount uint) (title (string-utf8 100)) (description (string-utf8 500)))
  (let
    ...
    (print { event: "proposal-created", proposal-id: new-proposal-id, proposer: tx-sender, amount: amount })
    (ok new-proposal-id)
  )
)

;; Vote on proposal - quadratic voting with cost deduction
(define-public (vote (proposal-id uint) (support bool) (vote-weight uint))
  (let
    ...
    (asserts! (> vote-weight u0) ERR-ZERO-AMOUNT)
    ...
    (print { event: "vote", proposal-id: proposal-id, voter: tx-sender, support: support, weight: vote-weight, cost: vote-cost })
    (ok true)
  )
)
...
(define-public (reclaim-vote-cost (proposal-id uint))
  (let
    ...
    (print { event: "vote-cost-reclaimed", proposal-id: proposal-id, voter: tx-sender, amount: (get cost-paid vote-data) })
    (ok true)
  )
)
...
(define-public (set-min-stake-amount (new-amount uint))
  (begin
    ...
    (print { event: "min-stake-updated", new-amount: new-amount, updated-by: tx-sender })
    (ok new-amount)
  )
)
